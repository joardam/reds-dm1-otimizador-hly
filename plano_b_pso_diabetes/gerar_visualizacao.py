...
def construir_dados():
    """Roda o pipeline real (PSO padrão + meta-FSS + gabarito sklearn) e devolve
    (payload, apres, meta). Reutilizado pelos geradores desktop e mobile, para não
    duplicar a execução do PSO nem o relevo de desempenho. `payload` é o JSON embutido
    na simulação; `apres` e `meta` são os HTML prontos das abas de apresentação e de
    hiperparâmetros (FSS)."""
    ...
    return payload, apres, meta


def render_html(template, payload, apres, meta):
    """Injeta os dados reais (JSON) e os HTML das abas no template escolhido."""
    return (template
...
def main():
    ...
    html = render_html(TEMPLATE, payload, apres, meta)
