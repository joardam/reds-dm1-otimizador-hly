...
<div class="wrap">
 ...
 <div class="panel">
  ...
  <div class="row"><label>Velocidade</label><input type="range" id="speed" min="1" max="10" value="2"></div>
...
let particles=[], gbest=null, iter=0, playing=false, traces=[];
...
function reset(){
 ...
 particles=[]; traces=[]; iter=0; gbest=null;
 for(let i=0;i<n;i++){
...
function loop(){ if(playing){ const sp=+document.getElementById('speed').value;
   for(let s=0;s<sp;s++) stepOnce(); draw(); } requestAnimationFrame(loop); }
// ---- controles ----
